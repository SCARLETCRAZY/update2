            ...
            if int(message.text) == product.code:
                ...
                if check == 1:
                    ...
                    if func.check_settings()[0][1] == '1':
                        ...
                        msg = bot.send_message(chat_id=message.chat.id,
                                     text=f'⚠️ Введите адрес доставки или напишите в поддержку о встрече с дропом')
    ...
    def create_product_5(message):
        try:
            if message.text == 'Yes':
                ...
                func.add_product_to_section(product_name, product.section, product.info)
                bot.send_message(
                    chat_id=message.chat.id,
